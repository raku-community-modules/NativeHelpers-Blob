...
role LinearArray[::T] does Positional[T] is export {
    ...
    sub calloc(size_t, size_t --> Pointer) is native(stdlib) {*}  # UNCOVERABLE
    ...
    method new(::?CLASS:U: Int $size) {
        with calloc($size, $sol) -> $storage {
            ...
            fail "Can't allocate memory";  # UNCOVERABLE
        }
    }
    ...
    method ASSIGN-POS(::?CLASS:D: $idx, T:D \st) {
        sub memmove(Pointer, Pointer, size_t) is native(stdlib) {*}  # UNCOVERABLE
    ...
    method dispose(::?CLASS:D:) {
        sub free(Pointer) is native(stdlib) {*}  # UNCOVERABLE
